    ...
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('opd.bab7.store') }}" method="POST">
                    @csrf
                    @method('PUT')
                    <!-- Nama Bab Field -->
                    <div class="form-group row mb-4">
                        ...
                        <div class="col-sm-12 col-md-4">
                            <input type="text" name="nama_bab" class="form-control" value="{{ old('nama_bab', $bab7->nama_bab) }}" required>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-12 col-md-4">
                            <select name="jenis_id" class="form-control selectric" required>
                                ...
                                @foreach($jenis as $item)
                                    <option value="{{ $item->id }}" {{ $bab7->jenis_id == $item->id ? 'selected' : '' }}>{{ $item->jenis }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-12 col-md-4">
                            <select name="tahun_id" class="form-control selectric" required>
                                ...
                                @foreach($tahun as $year)
                                    <option value="{{ $year->id }}" {{ $bab7->tahun_id == $year->id ? 'selected' : '' }}>{{ $year->tahun }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row mb-4">
                        ...
                        <div class="col-sm-12 col-md-4">
                            ...
                            value="{{ old('kode_opd', $bab7->kode_opd) }}" class="form-control" required>
                        <input type="text" name="nama_opd" id="nama_opd"
                            value="{{ old('nama_opd', $bab7->nama_opd) }}" class="form-control" disabled>
                        </div>
                    </div>
