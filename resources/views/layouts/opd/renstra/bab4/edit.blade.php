    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <a href="{{ route('layouts.opd.bab4.index') }}">
                        <button class="btn btn-primary"><i class="fa fa-arrow-left"></i> Back </button>
                    </a>
                    <form action="{{ route('opd.bab4.store') }}" method="POST">
                        @csrf
                        @method('PUT')
                        <!-- Nama Bab Field -->
                        <div class="form-group row mb-4">
                            ...
                            <div class="col-sm-12 col-md-4">
                                <input type="text" name="nama_bab" class="form-control"
                                    value="{{ old('nama_bab', $bab4->nama_bab) }}" required>
                            </div>
                        </div>
                                ...
                                <select name="jenis_id" class="form-control selectric" required>
                                    ...
                                    @foreach ($jenis as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $bab4->jenis_id == $item->id ? 'selected' : '' }}>{{ $item->jenis }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <select name="tahun_id" class="form-control selectric" required>
                                    ...
                                    @foreach ($tahun as $year)
                                        <option value="{{ $year->id }}"
                                            {{ $bab4->tahun_id == $year->id ? 'selected' : '' }}>{{ $year->tahun }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-group row mb-4">
                            ...
                            <div class="col-sm-12 col-md-4">
                                <input type="hidden" name="kode_opd" id="kode_opd"
                                    value="{{ old('kode_opd', $bab4->kode_opd) }}" class="form-control" required>
                                <input type="text" name="nama_opd" id="nama_opd"
                                    value="{{ old('nama_opd', $bab4->nama_opd) }}" class="form-control" disabled>
                            </div>
                        </div>
