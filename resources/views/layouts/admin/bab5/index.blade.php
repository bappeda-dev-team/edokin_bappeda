                        ...
                        <table class="table table-striped" id="table-1">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $bab_5->jenis->jenis ?? 'N/A' }}</td>
                                    <td>{{ $bab_5->tahun->tahun ?? 'N/A' }}</td>
                                    {{-- <td>{{ $bab_4->kode_opd->kode_opd ?? 'N/A' }}</td> --}}
                                    <td>
                                        @php
                                        // Ensure that urusan_opd is being processed correctly
                                            $kodeOpds = collect($urusan_opd['results'] ?? [])->firstWhere('kode_opd', $bab_5->kode_opd);
                                        @endphp
                                        {{ $kodeOpds['kode_opd'] ?? 'N/A' }}
                                    </td>
                                    <td>
                                        <a href="{{ route('bab5.show', $bab_5->id) }}" class="btn btn-info" data-id="{{ $bab_5->id }}"><i class="fa fa-eye"></i> Show</a>
                                        <a href="{{ route('bab5.edit', $bab_5->id) }}" class="btn btn-warning mx-2">Edit <i class="fas fa-edit"></i></a>
                                        <form action="{{ route('bab5.destroy', $bab_5->id) }}" method="POST" style="display:inline-block;">
                                            @csrf
                                            @method('DELETE')
