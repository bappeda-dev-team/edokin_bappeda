...
class Bab3Controller extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('layouts.admin.bab3.edit', compact('bab3', 'jenis', 'data_opd', 'tahun','isuStrategisList','permasalahanList'));
    }
    ...
    public function exportPdf($id)
    {
        try {
            ...
            $isuStrategisList = json_decode($bab3->isu_strategis, true);
            // Render HTML view
            $html = view('layouts.admin.bab3.pdf', compact('bab3', 'urusan_opd', 'permasalahanList', 'isuStrategisList'))->render();
    ...
    public function getPermasalahanOpd($tahun, $kode_opd)
    {
        try {
            ...
            if ($response->successful()) {
                ...
                $dataPermasalahanIsuStrategis = [];

                foreach ($programs as $program) {
                    $kode_program = $program['kode_program'] ?? 'Unknown';
                    $nama_program = $program['details'][0]['program'] ?? 'Unknown';
                    $isu_strategis = $program['details'][0]['isu_strategis'] ?? 'Unknown';

                    if (!isset($dataPermasalahanIsuStrategis[$kode_program])) {
                        $dataPermasalahanIsuStrategis[$kode_program] = [
                    ...
                    foreach ($program['details'] as $detail) {
                        $permasalahans = $detail['permasalahans'] ?? [];
                        foreach ($permasalahans as $permasalahan) {
                            if (!empty($permasalahan) && !in_array($permasalahan, $dataPermasalahanIsuStrategis[$kode_program]['permasalahans'])) {
                                $dataPermasalahanIsuStrategis[$kode_program]['permasalahans'][] = $permasalahan;
                            }
                        }
                    }
                }

                return response()->json(['results' => array_values($dataPermasalahanIsuStrategis)]);
            } else {
